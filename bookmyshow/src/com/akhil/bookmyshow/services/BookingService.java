...
import com.akhil.bookmyshow.models.ShowSeatState;
import com.akhil.bookmyshow.models.ShowSeatType;
import com.akhil.bookmyshow.models.User;
import com.akhil.bookmyshow.repositories.BookingRepository;
import com.akhil.bookmyshow.repositories.ShowSeatRepository;
...
public class BookingService {
	...
	public Booking createBooking(User user, Show show, List<Long> showSeatsIds) throws Exception {
		...
		// for one show only one request will be coming i.e. synchronizing over the show
		// object
		// synchronized (show) {
		...
		for (Long showSeatId : showSeatsIds) {
			toBeBookedSeatIds.add(showSeatId);
		}
		...
		if (!didGetAllLocks) {
			...
			throw new Exception("Some seats are unavailable");
		}

		// Step 3:Fetch the current status of those rows
		for (Long showSeatId : showSeatsIds) {
			currentShowSeats.add(showSeatRepository.getShowSeatById(showSeatId));
		}
		...
		for (ShowSeat currentShowSeat : currentShowSeats) {
			if (!currentShowSeat.getState().equals(ShowSeatState.AVAILABLE)) {
				...
				// If any of those seats have state of not available=> unlock all seats and
				// throw exception
				throw new Exception("All Seats Not available");
			}
		}

		// Step 5:Mark all of those seats as locked
		for (ShowSeat currentShowSeat : currentShowSeats) {
			currentShowSeat.setState(ShowSeatState.LOCKED);
			showSeatRepository.update(currentShowSeat);// Also update their state in database
		}
