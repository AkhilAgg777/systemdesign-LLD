...
import ridebookingappdesign.domain.NotificationMessage;
...
import ridebookingappdesign.domain.RideStatus;
import ridebookingappdesign.domain.RideStatusResponse;
import ridebookingappdesign.domain.Location;
import ridebookingappdesign.domain.state.RideStateMachine;
...
public class RideService {
    ...
    public Ride requestRide(RideRequest request) {
        ...
        double distanceKm = Math.max(0.5d,
                locationService.calculateDistanceKm(request.getPickupLocation(), request.getDropoffLocation()));
        long durationSec = Math.max(300L, locationService.estimateDurationSec(distanceKm));
        long estimatedFare = pricingService
                .estimateFare(request.getPickupLocation(), request.getDropoffLocation(), distanceKm, durationSec)
                .getEstimatedFare();

        Ride ride = new Ride(UUID.randomUUID().toString(), request.getRiderId(), request.getPickupLocation(),
                request.getDropoffLocation(), request.getPaymentType(), Instant.now().toEpochMilli());
        ...
        ride.setPaymentStatus(
                request.getPaymentType() == PaymentType.PRE_PAYMENT ? PaymentStatus.PENDING : PaymentStatus.NONE);
    ...
    public void completeRide(String rideId, String driverId) {
        withRideLock(rideId, () -> {
            ...
            double distanceKm = Math.max(0.5d,
                    locationService.calculateDistanceKm(ride.getPickupLocation(), ride.getDropoffLocation()));
            ride.setActualDistanceKm(distanceKm);
            long durationSec = Math.max(ride.getEstimatedDurationSec(),
                    locationService.estimateDurationSec(distanceKm));
    ...
    private void validateRequest(RideRequest request) {
        ...
        if (Math.abs(pickup.getLatitude() - drop.getLatitude()) < 0.0001
                && Math.abs(pickup.getLongitude() - drop.getLongitude()) < 0.0001) {
